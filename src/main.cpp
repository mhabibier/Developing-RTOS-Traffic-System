...
void initGPIO(void) {
    ...
    /* ── Tombol Emergency: INPUT (pull-up eksternal 10kΩ di diagram.json) ── */
    /* GPIO 34-39 tidak punya internal pull-up; mode INPUT cukup */
...
void setup() {
    /* ── 1. Serial Monitor ── */
    Serial.begin(115200);
    while (!Serial) { delay(10); } /* Tunggu Serial siap (USB CDC) */
